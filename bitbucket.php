...
function fetchRepository ()/*{{{ Fetch or clone repository */
{
	...
	if ( !is_dir($repoPath) || !is_file($repoPath.'HEAD') ) {
		...
		$exec = shell_exec('cd '.$CONFIG['repositoriesPath'].' && '.$CONFIG['gitCommand'].' clone --mirror git@bitbucket.org:'.$PROJECTS[$REPO]['repo_username'].'/'.$REPO.'.git');
        _LOG($exec);
	}
	// Else fetch changes
	else {
		_LOG("Fetching repository '$REPO'");
		$exec = shell_exec('cd '.$repoPath.' && '.$CONFIG['gitCommand'].' fetch');
	}
...
function checkoutProject ()/*{{{ Checkout project into target folder */
{
    ...
    $command = 'cd '.$repoPath.' && GIT_WORK_TREE='.$PROJECTS[$REPO]['projPath'].' '.$CONFIG['gitCommand'].' checkout -f '.$branch;
    _LOG('Command: '. $command);
	$exec = shell_exec($command);
    _LOG("Result: ".$exec);

	if ( !empty($PROJECTS[$REPO]['postHookCmd']) ) {
        $command = 'cd '.$PROJECTS[$REPO]['projPath'].' && '.$PROJECTS[$REPO]['postHookCmd'];
        _LOG("Command: ".$command);
		$exec = shell_exec($command);
        _LOG("Result: ".$exec);
	}
